"""CartpoleSwingup task for hydrax.

This module provides a hydrax task for the Cartpole Swingup environment
...
_XML_PATH = (
    ...
    / "mujoco_playground/mujoco_playground/_src/dm_control_suite/xmls/cartpole.xml"
)

# Checkpoint path for trained RL policy
CHECKPOINT_PATH = (
    Path(__file__).parent.parent.parent.parent.parent
    / "mujoco_playground/logs/CartpoleSwingup-20260125-141016/checkpoints"
)
...
class CartpoleSwingup(Task):
    """Cartpole swingup task with RL policy + SPC residuals.
    
    The goal is to swing up the pole and balance it upright while keeping
    the cart centered. Only the slider (cart) is actuated.
    """
    ...
    def __init__(
        ...
        """Initialize the CartpoleSwingup task.
        ...
        # Control is the slider motor
        self.nu = 1
        self.u_min = jnp.array([-1.0])
        self.u_max = jnp.array([1.0])
        
        # Get joint address for slider
        slider_jid = mujoco.mj_name2id(mj_model, mujoco.mjtObj.mjOBJ_JOINT, "slider")
        self._slider_qposadr = mj_model.jnt_qposadr[slider_jid]
        hinge_1_jid = mujoco.mj_name2id(mj_model, mujoco.mjtObj.mjOBJ_JOINT, "hinge_1")
        self._hinge_1_qposadr = mj_model.jnt_qposadr[hinge_1_jid]
    ...
    def reset(self, rng: jax.Array) -> "Tuple[mujoco.MjData, mjx.Data]":
        """Reset the environment with swing-up initial conditions.
        ...
        # Swing-up initial position: pole pointing down with small noise
        mj_data.qpos[self._slider_qposadr] = float(0.01 * jax.random.normal(rng1))
        mj_data.qpos[self._hinge_1_qposadr] = float(jnp.pi + 0.01 * jax.random.normal(rng2))
        
        # Small velocity noise
        mj_data.qvel[:] = 0.01 * jax.random.normal(rng3, (self.mj_model.nv,))
    ...
    def mjx_reset(self, rng: jax.Array) -> mjx.Data:
        ...
        # Swing-up initial position
        qpos = jnp.zeros(self.mj_model.nq)
        qpos = qpos.at[self._slider_qposadr].set(0.01 * jax.random.normal(rng1))
        qpos = qpos.at[self._hinge_1_qposadr].set(jnp.pi + 0.01 * jax.random.normal(rng2))
        
        # Small velocity noise
        qvel = 0.01 * jax.random.normal(rng3, (self.mj_model.nv,))
    ...
    def _get_obs(self, state: mjx.Data) -> jax.Array:
        ...
        Observation: cart_position (1) + pole_angle_cos (1) + pole_angle_sin (1) + qvel (2) = 5 dims
        """
        cart_position = state.qpos[self._slider_qposadr]
        pole_angle_cos = state.xmat[2, 2, 2]  # zz component of pole_1 (body index 2)
        pole_angle_sin = state.xmat[2, 0, 2]  # xz component of pole_1
        
        return jnp.concatenate([
            cart_position.reshape(1),
            pole_angle_cos.reshape(1),
            pole_angle_sin.reshape(1),
            state.qvel,
        ])
    ...
    def running_cost(self, state: mjx.Data, control: jax.Array) -> jax.Array:
        """Running cost for the cartpole swingup task.
        
        Cost based on upright position, centering, control effort, and velocity.
        """
        # Upright reward: pole_angle_cos = 1 when upright
        pole_angle_cos = state.xmat[2, 2, 2]
        upright = (pole_angle_cos + 1) / 2  # 0 to 1, 1 = upright
        
        # Centered reward: cart near origin
        cart_position = state.qpos[self._slider_qposadr]
        centered = jnp.exp(-0.5 * (cart_position / 2.0)**2)  # Gaussian tolerance, margin=2
        ...
        small_control = 1.0 - jnp.abs(total_action[0])
        small_control = jnp.clip((4 + small_control) / 5, 0.0, 1.0)
        
        # Small velocity reward
        angular_vel = state.qvel[1:]
        small_velocity = jnp.exp(-0.5 * jnp.sum(jnp.square(angular_vel / 5.0)))
        small_velocity = (1 + small_velocity) / 2
        
        # Combined reward (matching playground formula)
        total_reward = upright * small_control * small_velocity * centered
