...
class ConstrainedParticle(Task):
    """A particle task with a constraint to stay within a box.
    
    This task demonstrates CCEM (Constrained CEM) by requiring the particle
    to track a moving target while staying within a square box centered at
    the origin.
    """
    ...
    def __init__(self, box_size: float = 0.1) -> None:
        ...
        Args:
            box_size: Half-width of the box constraint (particle stays in [-size, size]).
        """
    ...
    def constraint_cost(self, state: mjx.Data, control: jax.Array) -> jax.Array:
        ...
        The magnitude of the cost increases with distance from the boundary.
        """
